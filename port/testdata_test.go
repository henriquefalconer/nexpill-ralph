...
// stringsData mirrors the testData.strings bucket in tests/tests.js:7-136.
// Fields: decoded (Unicode label), encoded (Punycode label, no xn-- prefix).
var stringsData = []struct {
	...
	// Russian: punycode.js uses lowercase-only (no mixed-case annotation)
	{"Russian (Cyrillic)",
...
// ucs2Data mirrors the testData.ucs2 bucket in tests/tests.js:137-175.
// decoded: code point sequence; encoded: UTF-16 code unit sequence.
var ucs2Data = []struct {
	description string
	decoded     []int32
	encoded     []uint16
	...
	{"Unmatched low surrogate, followed by a surrogate pair, followed by an unmatched low surrogate",
		[]int32{0xDC00, 0x1D306, 0xDC00},
		[]uint16{0xDC00, 0xD834, 0xDF06, 0xDC00}},
}

// domainsData mirrors the testData.domains bucket in tests/tests.js:176-220.
var domainsData = []struct {
	...
	{"foo\\x7F.example", "foo\x7F.example", "foo\x7F.example"},
}

// separatorsData mirrors the testData.separators bucket in tests/tests.js:221-242.
var separatorsData = []struct {
	description string
	decoded     string
	encoded     string
}{
	{"Using U+002E as separator", "ma\u00F1ana\u002Ecom", "xn--maana-pta.com"},
