...
0a. Study `specs/*` with up to 1 Sonnet subagent to learn the application specifications.
0b. Study @ralph/todo.md.

1. Your task is to implement functionality per the specifications and @ralph/todo.md file using parallel subagents. Follow @ralph/todo.md and choose the most important item to address. Before making changes, search the codebase (don't assume not implemented) using Sonnet subagents. You may use up to 1 Sonnet subagent for searches/reads and only 1 Sonnet subagent for build/tests. Use Opus subagents when complex reasoning is needed (debugging, architectural decisions).
